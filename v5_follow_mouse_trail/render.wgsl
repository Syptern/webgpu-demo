...
fn fs(in: Vout) -> @location(0) vec4f {
  ...
  // Indigo at rest -> cyan -> a touch of white only at the very fastest
  let t   = clamp(in.speed / 500.0, 0.0, 1.0);
  let tw  = pow(t, 5.0) * 0.4;          // less white, only at top speed
  let c0  = vec3f(0.12, 0.14, 0.90);
  let c1  = vec3f(0.05, 0.85, 1.00);
  let c2  = vec3f(1.00, 1.00, 1.00);
  let col = mix(mix(c0, c1, t), c2, tw);
